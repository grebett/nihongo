// Inline SVG icon library — Lucide-style strokes, 24x24 viewBox, 2px stroke.
// Use as `${icon('trash')}` in template strings; size/color via CSS on the parent.
...
export function icon(name: keyof typeof ICONS | string, size = 16): string {
  const body = ICONS[name as string];
  if (!body) return '';
